	...
	<div class="service-item <?php echo esc_attr( $icon_class . ' ' . $image_class ); ?>">
		<div class="service-thumbs">
			<?php if (!empty( $rt_service_icon )  ) { ?>
				<div class="service-icon" <?php echo wp_specialchars_decode( esc_attr( $service_bg ), ENT_COMPAT ); ?>><i class="<?php finwave_html( $rt_service_icon , false );?>"></i></div>
			<?php } ?>
				...
				<?php } if ( finwave_option( 'rt_service_read_more' ) ) { ?>
					<div class="rt-button">
						<a class="btn button-2" href="<?php the_permalink();?>">
							<?php esc_html_e('Read More' , 'finwave' ); ?><i class="icon-rt-right-arrow"></i>
						</a>
					</div>
				<?php } ?>
			</div>
